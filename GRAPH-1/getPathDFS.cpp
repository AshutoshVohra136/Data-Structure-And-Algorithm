...
#include <vector>
using namespace std;
vector<int>* getDFSPathHelper(bool** graph, int v, int start, int end,
bool* visited) {
if (start == end) {
...
vector<int>* smallOutput = getDFSPathHelper(graph, v, i,
end, visited);
if (smallOutput != NULL) {
...
vector<int>* output = getDFSPathHelper(graph, v, start, end,
visited);
delete[] visited;
return output;
}
